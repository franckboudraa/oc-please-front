  ...
  render() {
    ...
    return (
      <Table.Row>
        ...
        <Table.Cell>
          <Button.Group>
            <Button as={Link} to={`/r/${request.id}/edit`} color="green" icon="edit" />
            <Button as={Link} to={`/r/${request.id}/helpers`} color="blue" icon="users" />
            <Button onClick={() => this.setState({ modalOpen: true })} color="red" icon="delete" />
          </Button.Group>
          <Confirm
            open={modalOpen}
